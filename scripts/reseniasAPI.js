// CÓDIGO PARA QUE NO SE REPITAN LOS MISMOS COMENTARIOS
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function cargarLoteResenias() {
        ...
        let limite = Math.min(resultadoActual + 3, frasesPersonalizadas.length); // Calcula el límite de reseñas
        for (let i = resultadoActual; i < limite; i++) {
            traerDatosAPI(i);
        }

        // Actualizo el índice del resultado actual
        resultadoActual += 3;

        // Si ya se alcanzaron todas las reseñas, elimino el botón
        if (resultadoActual >= frasesPersonalizadas.length) {
